...
```bash
# ローカルで直接起動（go run。Docker不要で高速。開発イテレーション向け）
make dev
...
`make dev` は `go run ./cmd/server` でサーバーを直接起動する。Docker のフルビルドを挟まないため、コード変更後の起動が桁違いに速い。Python 分析を動かすにはローカルに **Python 3.11**（pip依存なし）が必要。

Firestore の接続先は環境変数で決まる:

| 環境変数 | 用途 |
| --- | --- |
| `FIRESTORE_DATABASE` | 接続先データベースID。**未設定なら Firestore 無効**で起動（スクレイピング結果はメモリ上のみ・永続化されない） |
| `FIRESTORE_EMULATOR_HOST` | 設定するとエミュレータに接続（例: `localhost:8081`）。**本番DBを誤って触らないため、ローカルではエミュレータ利用を推奨** |
| `GCP_PROJECT` / `GOOGLE_CLOUD_PROJECT` | プロジェクトID。メタデータサーバーの無いローカルではこれで `InitWithProjectID` 相当にフォールバックする。エミュレータ利用時はダミー値でよい |
| `APP_DIR` | `python3 scripts/analyze.py` を実行する作業ディレクトリ。未設定時は `/app`（Docker）が無ければカレントディレクトリを使う。通常プロジェクトルートから `make dev` するので設定不要 |

```bash
# Firestoreエミュレータ起動（要: gcloud components install cloud-firestore-emulator）
gcloud emulators firestore start --host-port=localhost:8081

# 別ターミナルでサーバー起動（エミュレータ接続・安全）
FIRESTORE_EMULATOR_HOST=localhost:8081 \
FIRESTORE_DATABASE=exvs-analyzer \
GCP_PROJECT=local-dev \
make dev

# Firestoreを使わず手早く起動（永続化なし）
make dev
```

プロジェクトID解決の優先順は `GCP_PROJECT` / `GOOGLE_CLOUD_PROJECT` → エミュレータ時のダミーID → Cloud Run メタデータサーバー。エミュレータ接続時は起動ログに `against emulator ...` と明示される。
